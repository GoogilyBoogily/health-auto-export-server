...
// Hours range for "evening" start time (main sleep usually starts between 8pm-2am)
const EVENING_START_HOUR = 20; // 8 PM
...
export function createSleepFrontmatter(
  ...
  if (mainSleepEntries.length > 0) {
    ...
    frontmatter.inBedDuration = roundTo(mainSleep.inBed / 60, 2); // minutes to hours
    frontmatter.asleepDuration = roundTo((mainSleep.core + mainSleep.deep + mainSleep.rem) / 60, 2);
    frontmatter.sleepEfficiency =
      mainSleep.inBed > 0
        ? Math.round(((mainSleep.core + mainSleep.deep + mainSleep.rem) / mainSleep.inBed) * 100)
        : undefined;

    // Sleep stage breakdown (convert minutes to hours)
    frontmatter.coreHours = roundTo(mainSleep.core / 60, 2);
    frontmatter.deepHours = roundTo(mainSleep.deep / 60, 2);
    frontmatter.remHours = roundTo(mainSleep.rem / 60, 2);
    frontmatter.awakeHours = roundTo(mainSleep.awake / 60, 2);
  ...
  if (napEntries.length > 0) {
    const napSessions: NapSession[] = napEntries.map((nap) => ({
      duration: roundTo((nap.core + nap.deep + nap.rem) / 60, 2), // minutes to hours
 ...
 * Sleep is attributed to the date of waking up (sleepEnd).
 */
export function groupSleepMetricsByDate(metricsByType: MetricsByType): Map<string, SleepMetric[]> {
  ...
  for (const sleep of sleepData) {
    // Use sleepEnd date for attribution (the day you wake up)
...
function isMainSleep(sleep: SleepMetric): boolean {
  const totalSleepHours = (sleep.core + sleep.deep + sleep.rem) / 60; // Convert minutes to hours
  const startHour = new Date(sleep.sleepStart).getHours();

  // Main sleep criteria:
  // 1. Duration > MIN_MAIN_SLEEP_HOURS
  // 2. Starts in evening (8pm-midnight) OR early morning (midnight-6am)
